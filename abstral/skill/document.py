    ...
    def create_sop_bench_seed(
        ...
        return cls(
            ...
            R=(
                f"Default topology reasoning for {topology_family} architecture.\n\n"
                f"- Start with a {topology_family} topology.\n"
                f"- The task domain is {domain} customer service with SOP compliance.\n"
                f"- Agents must verify all constraint prerequisites before executing actions.\n"
                f"- A ConstraintVerifier agent should check prerequisites before specialists act.\n"
                f"- Adjust based on observed trace failures."
            ),
            T="No agent templates defined yet. Templates will emerge from trace analysis.",
            P=(
                f"### Construction Protocol for SOPBench {domain.title()} Tasks\n\n"
                f"1. Always verify user identity (login) before any operations.\n"
                f"2. Check ALL constraint prerequisites before executing target actions.\n"
                f"3. Only ONE tool call at a time.\n"
                f"4. If a constraint check fails, decline the action and explain why.\n"
                f"5. Call exit_conversation when the request is completed or cannot be fulfilled.\n"
                f"6. Do NOT skip constraint verification steps even if user seems impatient."
            ),
            metadata={
                "version": "0",
